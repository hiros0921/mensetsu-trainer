...
public record ScoringPolicy(
  ...
  public Score evaluate(ScoreBreakdown breakdown) {
    ...
    for (Axis a : Axis.values()) {
      ...
      if (!s.measured()) {
        ...
        // 諏訪の判断（第5段階③）は「その選択にコストがあることを見せるべき」。
   ...
   * <h2>誰が何を決めたか</h2>
   *
   * 第5段階で3案を出し、諏訪が選んだ。案Aの重み・境目・帯に、測れなかった軸の扱いだけを
   * 「0点」に差し替えたもの。選定の理由は、諏訪の言葉で記録しておく。
   *
   * <ul>
   ...
   *
   * 諏訪は第5段階で「圧迫面接は一貫性が主軸」と述べている。その見立ては変わっていない。
   * 変わったのは、重みで表現するかどうか。
   *
   * <blockquote>「その軸が重要か」と「その軸に重みを置けるか」は、別の問題。
  ...
  public static ScoringPolicy adoptedPressure() {
    return new ScoringPolicy(
        ...
        // 【重要】画面に出る重みと、文言を一致させる。
        // 諏訪の指摘: 「案R3の一貫性25は具体性・沈黙と最大タイでした。
        //   画面の文言と数字が矛盾したのは、私の言い方が原因です。
        //   文言も実際の重みに合わせてください」
        // 「抑えてあります」は案R1（一貫性40）に対しての話で、表の中では最大タイ。
        // 表を見た人には、そう読めない。
        "重みは25で、具体性・沈黙と並んで最大です。ただし判定は LLM が行うので揺らぎます。"
            + "案R1の40まで上げなかったのはそのためです。"
