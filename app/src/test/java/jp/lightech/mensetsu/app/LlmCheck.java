...
public final class LlmCheck {
   ...
   * <h2>【重要】架空の人物です</h2>
   *
   * 仕様書10章「実在の企業名・個人名を、サンプルデータに含めないこと」。
   * テストコードも同じ扱いにする。
   *
   * <p>ここで測るのは応答時間（深掘りの最初の文字が出るまで）なので、
   * 中身が本物である必要はない。必要なのは<b>掘れる材料が入っていること</b>だけ。
   * 技術用語が出て、選定理由と比較対象が語られていれば、面接官は3段掘れる。
   */
  private static final List<String> SCRIPT =
      List.of(
          "受託開発の会社でバックエンドを担当しています。直近は在庫管理の SaaS を"
              + "Java と Spring Boot で作り、データベースは PostgreSQL を使っています。",
          "データベースは私が選びました。取引先名のあいまい一致に pg_trgm を使いたかったのと、"
              + "チームに運用経験があったためです。",
